...
import { ChevronLeft, Sparkles, ScrollText, FileText, Layout } from 'lucide-react';
import VisualEngine from '../../components/VisualEngine';
import UnifiedPDFList from '../../components/UnifiedPDFList';
import data from '../../data/tnpsc/group1.json';
...
const Group1 = () => {
  ...
  const formatFiles = (files) => files.map(f => ({ name: f, title: f.replace('.pdf', '').replace(/_/g, ' ') }));
    ...
    <div className="relative w-full min-h-screen pt-24 pb-12 px-6 md:px-12 bg-[#F4F7F5] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex items-center gap-2 mb-10 bg-white/40 p-2 rounded-[24px] border border-[#DAD7CD]/30 w-fit backdrop-blur-xl shadow-sm">
          {[
            { id: 'hub', name: 'Overview', icon: ScrollText, path: '/notes/tnpsc/group1' },
            { id: 'pyq', name: 'PYQ Archives', icon: FileText, path: '/notes/tnpsc/group1/pyq' },
            { id: 'syllabus', name: 'Syllabus', icon: ScrollText, path: '/notes/tnpsc/group1/syllabus' },
          ].map((t) => (
            <button
              key={t.id}
        ...
        <AnimatePresence mode="wait">
            {activeTab === 'hub' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div onClick={() => navigate('/notes/tnpsc/group1/pyq')} className="p-10 bg-white rounded-[40px] border border-[#DAD7CD]/30 shadow-sm hover:shadow-xl transition-all cursor-pointer group">
                    ...
                    <p className="text-[#6B7A7A] leading-relaxed">Collection of question papers from 2011 to 2025.</p>
                 </div>
                 <div onClick={() => navigate('/notes/tnpsc/group1/syllabus')} className="p-10 bg-white rounded-[40px] border border-[#DAD7CD]/30 shadow-sm hover:shadow-xl transition-all cursor-pointer group">
                    <div className="w-16 h-16 rounded-2xl bg-[#F4F7F5] flex items-center justify-center text-[#588157] mb-8 group-hover:bg-[#588157] group-hover:text-white transition-all">
                       <ScrollText size={32} />
                    </div>
                    <h3 className="text-[22px] font-bold text-[#2F3E46] mb-3">Group 1 Syllabus</h3>
                    <p className="text-[#6B7A7A] leading-relaxed">Official Prelims and Mains components with marking schemes.</p>
                 </div>
              </motion.div>
            )}
            {activeTab === 'pyq' && (
                <motion.div key="pyq" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <UnifiedPDFList title="PYQ Archives" type="pyq" group="group1" files={formatFiles(data.pyqFiles)} />
                </motion.div>
            )}
            {activeTab === 'syllabus' && (
                <motion.div key="syllabus" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <UnifiedPDFList title="Official Syllabus" type="syllabus" group="group1" files={formatFiles(data.syllabusFiles)} />
                </motion.div>
            )}
        </AnimatePresence>
      </div>
